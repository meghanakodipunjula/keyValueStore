...
public class CRDHelper {
    ...
    public static boolean isKeyNameValid(String key) {
        if (key.length() > Constants.MAX_KEY_LENGTH) {
            return false;
        }
        return true;
    }

    public static boolean doesFileSizeLimitExceed(File file) {
    ...
    public static boolean isKeyExists(String key, String filePath) {
        ...
        try {
            ...
            // validate against time to live and destroy the object if time expires
            if (isKeyExists) {
                ...
                {
                    // the object has expired - removing it
                    dataMap.remove(key);
                    ...
                    // As object has been removed , now the key is available for storage
                    isKeyExists = false;
                }
            }
    ...
    /**
     * Delete value from the DataStore
     *
     * @param key
